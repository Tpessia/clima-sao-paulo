...
if (isset($_GET["qnt"])) {
    ...
    if (isset($_GET["date"])) {
        ...
        $last = file_get_contents("https://www.thiagopessia.com/clima-sao-paulo/get-last.php");
    ...
    for ($i = $qnt; $i > 0; $i--) {
        ...
        if ($dif > 0) {
            ...
            if (isset($data["history"]["dailysummary"][0]) && !(intval($data["history"]["dailysummary"][0]["date"]["year"]) == 1969 && intval($data["history"]["dailysummary"][0]["date"]["mon"]) == 12 && intval($data["history"]["dailysummary"][0]["date"]["mday"]) == 31)) {
                echo do_post_request("https://www.thiagopessia.com/clima-sao-paulo/insert-daily.php", http_build_query(summaryData($data)));
            }
            else {
                echo do_post_request("https://www.thiagopessia.com/clima-sao-paulo/insert-daily.php", http_build_query(array(
            ...
            if (isset($data["history"]["observations"][0])) {
                do_post_request("https://www.thiagopessia.com/clima-sao-paulo/insert-hours.php", http_build_query(hourlyData($data)));
            }
            else {
                do_post_request("https://www.thiagopessia.com/clima-sao-paulo/insert-hours.php", http_build_query(array(
